# PROMPT — Cole este texto inteiro no chat de qualquer app do cliente

---

Você é o dev sênior responsável por este projeto. Preciso que você aplique a padronização completa de identidade visual sem quebrar nenhuma funcionalidade existente.

## O QUE FAZER

1. Substituir qualquer animação de fundo da tela de login (canvas, gradiente, SplashCursor, ShapeGrid, etc) pelo componente `Hyperspeed` abaixo
2. Reescrever a tela de login seguindo exatamente o template abaixo (mantendo a lógica de autenticação que já existe)
3. Aplicar os tokens CSS exatos no `src/index.css`
4. Não alterar nada fora da tela de login e do index.css

---

## PASSO 1 — Instalar dependências (se não tiver)
...
## PASSO 2 — Criar/substituir `src/components/Hyperspeed.tsx` com o código abaixo
...
## PASSO 3 — Substituir a tela de login

Mantenha toda a lógica de autenticação que já existe (signIn, useAuth, navigate, etc).
Troque apenas o visual/layout pelo template abaixo. Adapte só:
- `APP_TITLE` → nome deste painel específico
- `EMAIL_DOMAIN` → domínio que já é usado neste app
- A chamada de `signIn` / `supabase.auth` → use exatamente o que já existe no app

```tsx
import { useState, useMemo } from "react";
// mantenha seus imports de auth/navigate existentes
import Hyperspeed from "@/components/Hyperspeed";

const APP_TITLE = "NOME DO PAINEL"; // ← ALTERE AQUI
const EMAIL_DOMAIN = "@dominio.existente.com"; // ← use o que já existe no app
...
export default function Login() {
  ...
  // mantenha useNavigate / useAuth que já existem
...
## PASSO 4 — Atualizar tokens de cor no `src/index.css`

Substitua apenas o bloco `:root` e `.dark` (mantenha o resto do arquivo):
...
Além disso, adicione (ou confirme que já existe) no `@layer base`:
...
- Não altere nada além dos 4 passos acima
- Não quebre nenhuma rota, hook, contexto ou lógica existente
- Se o app não tiver `three` ou `postprocessing` instalados, instale antes de iniciar
- Se o componente `Hyperspeed` já existir com outro nome, substitua o arquivo e atualize o import na tela de login
- Após aplicar, verifique se o app ainda builda sem erros (`npm run build`)
